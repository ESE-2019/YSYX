...
int main(int argc, char **argv)
{
    ...
    while (!contextp->gotFinish() && ebreak_n && (cycle_ipc < (uint64_t)ABORT_NUM || ABORT_NUM == 0))
    {
        ...
        if (!top->clock)
        {
            ...
            else
            {
                ...
                {
                    for (int i = 0; i < 16; i++)
                        if (top->rootp->top__DOT__CORE__DOT__REG__DOT__regfile[i] != reg_mirror[i])
                        {
                            reg_mirror[i] = top->rootp->top__DOT__CORE__DOT__REG__DOT__regfile[i];
                            fprintf(log_file, "\treg[%02d]: 0x%08x\n", i,
                                    top->rootp->top__DOT__CORE__DOT__REG__DOT__regfile[i]);
                        }
                }
    ...
    if (SDB || LOG)
    {
        ...
        for (int i = 0; i < 16; i++)
            printf("x%02d: 0x%08x\n", i, top->rootp->top__DOT__CORE__DOT__REG__DOT__regfile[i]);
        printf("mstatus, mtvec, mepc, mcause\n");
        for (int i = 0; i < 4; i++)
            printf("csr: 0x%08x\n", top->rootp->top__DOT__CORE__DOT__CSR__DOT__regfile[i]);
    }
    // Final model cleanup
    top->final();
    fclose(log_file);
    ...
    double ipc_num;
    ipc_num = (double)inst_ipc / (double)cycle_ipc;
