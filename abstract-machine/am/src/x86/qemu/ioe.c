...
static void uart_config(AM_UART_CONFIG_T *cfg) { cfg->present = true; }
...
static void input_config(AM_INPUT_CONFIG_T *cfg) { cfg->present = true; }
...
static void gpu_status(AM_GPU_STATUS_T *stat) { stat->ready = true; }
...
static struct pixel *render(struct gpu_canvas *cv, struct gpu_canvas *parent,
                            struct pixel *px) {
  ...
  case AM_GPU_SUBTREE: {
    ...
    for (struct gpu_canvas *ch = to_host(cv->child); ch;
         ch = to_host(ch->sibling)) {
...
static void disk_status(AM_DISK_STATUS_T *status) { status->ready = true; }
...
static void *lut[128] = {
    [AM_UART_CONFIG] = uart_config,   [AM_UART_TX] = uart_tx,
    [AM_UART_RX] = uart_rx,           [AM_TIMER_CONFIG] = timer_config,
    [AM_TIMER_RTC] = timer_rtc,       [AM_TIMER_UPTIME] = timer_uptime,
    [AM_INPUT_CONFIG] = input_config, [AM_INPUT_KEYBRD] = input_keybrd,
    [AM_GPU_CONFIG] = gpu_config,     [AM_GPU_FBDRAW] = gpu_fbdraw,
    [AM_GPU_STATUS] = gpu_status,     [AM_GPU_MEMCPY] = gpu_memcpy,
    [AM_GPU_RENDER] = gpu_render,     [AM_AUDIO_CONFIG] = audio_config,
    [AM_DISK_CONFIG] = disk_config,   [AM_DISK_STATUS] = disk_status,
