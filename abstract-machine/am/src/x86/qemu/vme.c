...
static uintptr_t baseof(uintptr_t addr) { return addr & ~(mmu.pgsize - 1); }
  ...
  for (int i = 0; i < LENGTH(vm_areas); i++) {
    ...
    if (vma->kernel) {
      for (uintptr_t cur = (uintptr_t)vma->area.start;
           cur != (uintptr_t)vma->area.end; cur += mmu.pgsize) {
...
void protect(AddrSpace *as) {
  ...
  for (int i = 0; i < LENGTH(vm_areas); i++) {
    ...
    if (vma->kernel) {
      ...
      for (uintptr_t cur = (uintptr_t)vma->area.start;
           cur != (uintptr_t)vma->area.end; cur += (1L << info->shift)) {
...
void unprotect(AddrSpace *as) { teardown(0, (void *)&as->ptr); }
...
void map(AddrSpace *as, void *va, void *pa, int prot) {
  ...
  if (prot == MMAP_NONE) {
    ...
    *ptentry = 0;
    ...
    uintptr_t pte =
        (uintptr_t)pa | PTE_P | PTE_U | ((prot & MMAP_WRITE) ? PTE_W : 0);
    *ptentry = pte;
  }
