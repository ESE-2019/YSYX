...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)                \
  ...
  INSTPAT("??????? ????? ????? 110 ????? 11100 11", csrrsi, I, IFDEF(CONFIG_ETRACE, Log("csrrsi")); R(rd) = tmp; CSR_w(imm, uimm | tmp););
  INSTPAT("??????? ????? ????? 111 ????? 11100 11", csrrci, I, IFDEF(CONFIG_ETRACE, Log("csrrci")); R(rd) = tmp; CSR_w(imm, (~uimm) | tmp););
