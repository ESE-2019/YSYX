...
static void
sdcard_io_handler (uint32_t offset, int len, bool is_write)
{
  int idx = offset / 4;
  switch (idx)
    {
      ...
      else if (fp)
	{
      ...
      break;
    default:
      Log ("offset = 0x%x(idx = %d), is_write = %d, data = 0x%x", offset,
	   idx, is_write, base[idx]);
...
init_sdcard ()
{
  ...
  add_mmio_map ("sdhci", CONFIG_SDCARD_CTL_MMIO, base, 0x80,
		sdcard_io_handler);
