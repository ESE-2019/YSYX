...
uint8_t *
guest_to_host (paddr_t paddr)
{
  return pmem + paddr - CONFIG_MBASE;
}

paddr_t
host_to_guest (uint8_t * haddr)
{
...
static void
out_of_bound (paddr_t addr)
{
  panic ("address = " FMT_PADDR " is out of bound of pmem [" FMT_PADDR
	 ", " FMT_PADDR "] at pc = " FMT_WORD, addr, PMEM_LEFT,
	 PMEM_RIGHT, cpu.pc);
}
...
word_t
paddr_read (paddr_t addr, int len)
{
  #ifdef CONFIG_MTRACE
...
void
paddr_write (paddr_t addr, int len, word_t data)
{
  #ifdef CONFIG_MTRACE
    Log("paddr_write 0x%08x", addr);
  #endif
  if (likely (in_pmem (addr)))
    {
