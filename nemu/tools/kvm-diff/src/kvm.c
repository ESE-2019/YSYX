...
#define RFLAGS_FIX_MASK                                                        \
  (RFLAGS_ID | RFLAGS_AC | RFLAGS_RF | RFLAGS_TF | RFLAGS_AF)
...
static void kvm_set_step_mode(bool watch, uint32_t watch_addr) {
  ...
  debug.arch.debugreg[7] =
      (watch ? 0x1 : 0x0); // watch instruction fetch at `watch_addr`
...
static int patching() {
  ...
  if (vm.mem[pc] == 0x9c) { // pushf
    ...
    *(uint32_t *)(vm.mem + esp) =
        vcpu.kvm_run->s.regs.regs.rflags & ~RFLAGS_FIX_MASK;
    ...
    if (vcpu.int_wp_state == STATE_INT_INST)
      return 0;
    uint32_t esp = va2pa(vcpu.kvm_run->s.regs.regs.rsp);
    vcpu.kvm_run->s.regs.regs.rflags =
        *(uint32_t *)(vm.mem + esp) | RFLAGS_TF | 2;
    vcpu.kvm_run->s.regs.regs.rsp += 4;
    vcpu.kvm_run->s.regs.regs.rip++;
    vcpu.kvm_run->kvm_dirty_regs = KVM_SYNC_X86_REGS;
    return 1;
...
static void kvm_exec(uint64_t n) {
  for (; n > 0; n--) {
    ...
    if (vcpu.kvm_run->exit_reason != KVM_EXIT_DEBUG) {
      ...
      fprintf(
          stderr,
          "Got exit_reason %d at pc = 0x%llx, expected KVM_EXIT_DEBUG (%d)\n",
      ...
      if (vcpu.int_wp_state == STATE_INT_INST) {
        ...
        uint32_t eflag_addr =
            va2pa(vcpu.kvm_run->s.regs.regs.rsp + eflag_offset);
        *(uint32_t *)(vm.mem + eflag_addr) &= ~RFLAGS_FIX_MASK;

        Assert(vcpu.entry == vcpu.kvm_run->debug.arch.pc,
               "entry not match, right = 0x%llx, wrong = 0x%x",
               vcpu.kvm_run->debug.arch.pc, vcpu.entry);
        kvm_set_step_mode(false, 0);
        vcpu.int_wp_state = STATE_IDLE;
        // Log("exception = %d, pc = %llx, dr6 = %llx, dr7 = %llx",
        // vcpu.kvm_run->debug.arch.exception,
        //     vcpu.kvm_run->debug.arch.pc, vcpu.kvm_run->debug.arch.dr6,
        //     vcpu.kvm_run->debug.arch.dr7);
